...
const loginUser = asyncHandler(async(req, res) => {
    ...
    const user = await User.findOne({email}).populate({path: 'boards', select: '_id name icon groups groupInvites', populate: {path: 'friends friendRequests adminGroups', select: 'name _id icon requests'}})
...
const getUser = async(req, res) => {
    ...
    const user = await User.findById(id).populate({path: 'boards', select: '_id name icon groups groupInvites', populate: {path: 'friends friendRequests adminGroups', select: 'name _id icon requests'}})
...
const getAllUsers = async(req, res) => {
    const users = await User.find({}).sort({name: 1}).populate('boards', 'icon')
